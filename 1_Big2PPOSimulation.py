...
class big2PPOSimulation(object):
    ...
    def run(self):
        ...
        for _ in range(self.n_steps):
            ...
            print(f"mb_obs shape: {np.array(mb_obs).shape}")
            print(f"mb_pGos shape: {np.array(mb_pGos).shape}")
            print(f"mb_availAcs shape: {np.array(mb_availAcs).shape}")
            print(f"mb_actions shape: {np.array(mb_actions).shape}")
            print(f"mb_values shape: {np.array(mb_values).shape}")
            print(f"mb_neglogpacs shape: {np.array(mb_neglogpacs).shape}")
            print(f"mb_dones shape: {np.array(mb_dones).shape}")
            print(f"mb_rewards shape: {np.array(mb_rewards).shape}")
            # Log minibatch data to CSV after each step
            with open('minibatch_log.csv', 'a', newline='') as f:
                writer = csv.writer(f)
                # Write headers if file is empty
                if f.tell() == 0:
                    writer.writerow(['step', 'obs_shape', 'gos_shape', 'avail_acs_shape', 
                                    'actions_shape', 'values_shape', 'neglogpacs_shape', 
                                    'dones_shape', 'rewards_shape'])
                writer.writerow([_, 
                                np.array(mb_obs).shape,
                                np.array(mb_pGos).shape,
                                np.array(mb_availAcs).shape,
                                np.array(mb_actions).shape,
                                np.array(mb_values).shape,
                                np.array(mb_neglogpacs).shape,
                                np.array(mb_dones).shape,
                                np.array(mb_rewards).shape])
