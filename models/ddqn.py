...
class Memory:
    def __init__(self, len):
        ...
        self.avail_actions = collections.deque(maxlen=len)  # Added for action masking

    def update(self, state, action, reward, done, avail_actions):
        if not done:
...
def train(batch_size, current, target, optim, memory, gamma):
    states, actions, next_states, rewards, is_done, avail_actions, next_avail_actions = memory.sample(batch_size)
    ...
    next_q_values = next_q_values + (next_avail_actions - 1) * 1e8
    next_q_state_values = next_q_state_values + (next_avail_actions - 1) * 1e8
...
# def main(gamma=0.995,  # Increased for longer-term planning
#     lr=2.5e-4,    # Matched with your PPO implementation
#     min_episodes=50,  # Increased for more initial exploration
#     eps=1.0,
#     eps_decay=0.9995,  # Slower decay for more exploration
#     eps_min=0.01,
#     update_step=5,  # More frequent updates
#     batch_size=64,
#     update_repeats=10,
#     num_episodes=10000,
#     max_memory_size=50000,  # Increased for more experience replay
