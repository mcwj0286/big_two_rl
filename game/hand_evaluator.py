...
def evaluate_five_card_hand(cards):
    ...
    if is_straight and is_flush:
        ...
        # Four of a Kind
        four_rank = max(rank_count, key=lambda k: rank_count[k] if rank_count[k] == 4 else -1)
        ...
        # Full House
        triple_rank = max(rank_count, key=lambda k: (rank_count[k], Card.rank_order[k]))
...
def compare_hands(hand1, hand2):
    ...
    if type1 in [HandType.SINGLE, HandType.PAIR, HandType.TRIPLE, HandType.STRAIGHT, HandType.FLUSH, HandType.STRAIGHT_FLUSH]:
        ...
        return Card.rank_order[key1] - Card.rank_order[key2]
